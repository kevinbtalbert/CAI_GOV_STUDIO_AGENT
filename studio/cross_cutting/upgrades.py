...
def check_studio_upgrade_status(
    ...
    return CheckStudioUpgradeStatusResponse(
        ...
        out_of_date=not (local_commit == remote_commit)
    )
...
def upgrade_studio(
    ...
    restart_studio_application(
        RestartStudioApplicationRequest(),
        cml=cml,
        dao=dao
    )
...
def restart_studio_application(
    ...
    application: cmlapi.Application = get_appliction_by_name(
        cml=cml,
        name=AGENT_STUDIO_SERVICE_APPLICATION_NAME
    )
    
    # Restart the application
    cml.restart_application(
